      ...
      <div className="w-full flex flex-col gap-12 justify-between items-center h-full">
        ...
        <section className="w-full flex flex-wrap gap-4 max-w-6xl justify-center items-start overflow-y-auto">  
              {useCases.map((useCase, index) => (
                    <div className="w-72 min-h-64 max-h-64 flex flex-col justify-start items-start border border-slate-300 rounded-md bg-slate-50 overflow-hidden" key={index}>  
                      <div className="w-full font-bold text-slate-700 p-3 ps-5 border-b border-slate-300 bg-slate-100">
                          {useCase.title}
                      </div> 
                      <div className="w-full h-full grow flex flex-col justify-between items-between"></div>
                        <ul className="w-full h-full flex flex-col justify-between items-between ps-5 pe-4 gap-2 p-3">
                          {
