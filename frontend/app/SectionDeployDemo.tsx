...
export function SectionDeployDemo() {
  ...
  // Reset current org index if it becomes invalid after filtering
  useEffect(() => {
    if (currentOrgIndex >= availableOrganizations.length) {
  ...
  console.log("currentOrg: ", { currentOrg, currentOrgIndex });
  ...
  const callConstitute = useCallback(
    async (powersAddress: `0x${string}`) => {  
      ...
      try {
        ...
        console.log("Calling constitute with law data:", lawInitData);
              ...
              <div className="w-full sm:w-fit h-12 order-2 sm:order-1">
                {constituteCompleted && receipt?.contractAddress ? (
                  ...
                  <button 
                    ...
                    disabled={!ready || !authenticated || currentOrg.metadata.disabled || !areRequiredFieldsFilled() || status === 'error'}
                  > 
                    {status === 'error' ? (
                      'Error'
