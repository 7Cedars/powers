...
export const Voting = ({ powers, status: statusPowers}: {powers: Powers | undefined, status: Status}) => {
  ...
  const { action: actionWithVotes, fetchVoteData } = useAction()
  ...
  // Fetch role holders count
  useEffect(() => {
    const fetchVoteDataAction = async () => {
      if (action?.actionId && powers?.contractAddress) {
        const actionWithVotes = await fetchVoteData(action, powers)
        if (actionWithVotes) {
          setAction(actionWithVotes)
        } else {
    ...
    fetchVoteDataAction()
  }, [action?.lawId, powers?.contractAddress, chainId])

  // Use updated action data if available, otherwise use prop
  const allVotes = Number(action?.forVotes || 0) + Number(action?.againstVotes || 0) + Number(action?.abstainVotes || 0)
  ...
  const deadline = Number(action?.voteEnd || 0)
  const state = action?.state ?? 6
      ...
      <section className="w-full flex flex-col divide-y divide-slate-300 text-sm text-slate-600 border border-slate-300 rounded-md overflow-hidden" > 
        ...
        <div className = "w-full flex flex-col justify-center items-center gap-2 py-2 px-4"> 
          <div className = "w-full flex flex-row justify-between items-center">
            { Number(action?.forVotes || 0) + Number(action?.abstainVotes || 0) >= quorum ? <CheckIcon className="w-4 h-4 text-green-600"/> : <XMarkIcon className="w-4 h-4 text-red-600"/>}
            <div>
            { Number(action?.forVotes || 0) + Number(action?.abstainVotes || 0) >= quorum ? "Quorum passed" : "Quorum not passed"}
            </div>
          </div>
          <div className={`relative w-full leading-none rounded-sm h-3 border border-slate-300 overflow-hidden`}>
            <div 
              ...
              style={{width:`${quorum > 0 ? ((Number(action?.forVotes || 0) + Number(action?.abstainVotes || 0)) * 100) / quorum : 0 }%`}}> 
            </div>
          </div>
          <div className="w-full text-sm text-left text-slate-500"> 
           {roleHolders > 0 ? `${Number(action?.forVotes || 0) + Number(action?.abstainVotes || 0) } / ${quorum} votes` : "Loading..."}
          </div>
        </div>
        ...
        <div className = "w-full flex flex-col justify-center items-center gap-2 py-2 px-4"> 
          <div className = "w-full flex flex-row justify-between items-center">
            { Number(action?.forVotes || 0) >= threshold ? <CheckIcon className="w-4 h-4 text-green-600"/> : <XMarkIcon className="w-4 h-4 text-red-600"/>}
            <div>
            { Number(action?.forVotes || 0) >= threshold ? "Threshold passed" : "Threshold not passed"}
            </div>
          </div>
          <div className={`relative w-full flex flex-row justify-start leading-none rounded-sm h-3 border border-slate-300`}>
            ...
            <div className={`absolute bottom-0 w-full leading-none h-3 bg-red-400`} style={{width:`${allVotes > 0 ? ((Number(action?.forVotes || 0) + Number(action?.againstVotes || 0)) / allVotes)*100 : 0}%`}} />
            <div className={`absolute bottom-0 w-full leading-none h-3 bg-green-400`} style={{width:`${allVotes > 0 ? ((Number(action?.forVotes || 0)) / allVotes)*100 : 0}%`}} />
          ...
          <div className="w-full flex flex-row justify-between items-center"> 
            <div className="w-fit text-sm text-center text-green-500">
              {roleHolders > 0 ? `${Number(action?.forVotes || 0)} for` : "na"}
            </div>
            <div className="w-fit text-sm text-center text-red-500">
              {roleHolders > 0 ? `${Number(action?.againstVotes || 0)} against` : "na"}
            </div>
            <div className="w-fit text-sm text-center text-gray-500">
            {roleHolders > 0 ? `${Number(action?.abstainVotes || 0)} abstain` : "na"}
            </div>
          </div>
        </div>
