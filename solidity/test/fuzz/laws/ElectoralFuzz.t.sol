...
contract ElectoralFuzzTest is TestSetupElectoral {
    ...
    function testFuzzRoleByRolesWithRoleCombinations(
        ...
        (returnedActionId, returnedTargets, , returnedCalldatas) = roleByRoles.handleRequest(accountFuzzed, address(daoMock), 9, lawCalldata, nonceFuzzed); 
        ...
        for (i = 0; i < numberOfAccounts; i++) {
            if (accountsFuzzed[i] != address(0)) {
                (returnedActionId, returnedTargets, , ) = 
                    roleByRoles.handleRequest(accountsFuzzed[i], address(daoMock), 9, abi.encode(accountsFuzzed[i]), nonceFuzzed);
    ...
    function testFuzzVoteInOpenElectionWithMultipleVotes(
        ...
        for (j = 0; j < 3;) { // Vote three times max
    ...
    function testFuzzTaxSelectWithVariousAccounts(
        ...
        (returnedActionId, returnedTargets, , returnedCalldatas) = taxSelect.handleRequest(accountFuzzed, address(daoMock), 4, lawCalldata, nonceFuzzed);
    ...
    function testFuzzTaxSelectThresholdEdgeCases(
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(Erc20Taxed.getTaxLogs.selector),
            abi.encode(actualTaxPaid)
        );
        ...
        lawCalldata = abi.encode(accountFuzzed);        
        (returnedActionId, returnedTargets, , ) = taxSelect.handleRequest(accountFuzzed, address(daoMock), 4, lawCalldata, nonceFuzzed); 
    ...
    function testFuzzBuyAccessWithVariousPayments(
        ...
        (returnedActionId, returnedTargets, , returnedCalldatas) = buyAccess.handleRequest(accountFuzzed, address(daoMock), 5, lawCalldata, nonceFuzzed);
    ...
    function testFuzzNStrikesRevokesRolesThreshold(
        ...
        (returnedActionId, returnedTargets, , ) = nStrikesRevokesRoles.handleRequest(
            accountFuzzed, address(daoMock), 8, lawCalldata, nonceFuzzed
        ); 
