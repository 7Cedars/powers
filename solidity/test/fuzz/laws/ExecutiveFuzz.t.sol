...
contract ExecutiveFuzzTest is TestSetupExecutive {
    ...
    function testFuzzGovernorCreateProposalWithEmptyArrays(
        string memory descriptionFuzzed,
        uint256 nonceFuzzed
    ) public {
        vm.assume(bytes(descriptionFuzzed).length > 0);
    ...
    function testFuzzGovernorCreateProposalWithEmptyDescription(
        uint256 arrayLength,
        uint256 nonceFuzzed
    ) public {
        arrayLength = bound(arrayLength, 1, MAX_FUZZ_TARGETS);
    ...
    function testFuzzGovernorExecuteProposalWithEmptyArrays(
        string memory descriptionFuzzed,
        uint256 nonceFuzzed
    ) public {
        vm.assume(bytes(descriptionFuzzed).length > 0);
    ...
    function testFuzzGovernorExecuteProposalWithEmptyDescription(
        uint256 arrayLength,
        uint256 nonceFuzzed
    ) public {
        arrayLength = bound(arrayLength, 1, MAX_FUZZ_TARGETS);
    ...
    /// @notice Fuzz test AdoptLaws (lawId 4) returns preset adoption data regardless of input
    /// @dev lawId 4 is configured to adopt a single PresetSingleAction law
    function testFuzzAdoptLawsIgnoresInput(
    ...
    /// @notice Fuzz test AdoptLaws with various nonces
    function testFuzzAdoptLawsWithVariousNonces(
        uint256 nonce1,
        uint256 nonce2
    ) public {
        ...
        (returnedActionId, returnedTargets, , ) = 
            adoptLaws.handleRequest(alice, address(daoMock), 4, lawCalldata, nonce1);
        
        uint256 firstActionId = returnedActionId;
        address[] memory firstTargets = returnedTargets;
        
        (returnedActionId, returnedTargets, , ) = 
            adoptLaws.handleRequest(alice, address(daoMock), 4, lawCalldata, nonce2);
    ...
    function testFuzzActionIdConsistency(
        ...
        (returnedActionId, , , ) = statementOfIntent.handleRequest(alice, address(daoMock), lawIdFuzzed, lawCalldata, nonceFuzzed);
    ...
    function testFuzzGovernorContractValidation(
        uint256 arrayLength,
        uint256 nonceFuzzed
    ) public {
        // Bound inputs
    ...
    function testFuzzNonceUniqueness(
        ...
        (returnedActionId, , , ) = statementOfIntent.handleRequest(alice, address(daoMock), lawId, lawCalldata, nonce1Fuzzed);
        uint256 firstActionId = returnedActionId;
        
        (returnedActionId, , , ) = statementOfIntent.handleRequest(alice, address(daoMock), lawId, lawCalldata, nonce2Fuzzed);
