...
contract DeploySeparatedPowers is Script {
        ...
        returns (address[] memory targets, uint256[] memory values, bytes[] memory calldatas)
    {
        ...
        calldatas[0] = abi.encodeWithSelector(IPowers.assignRole.selector, 3, alice);
        calldatas[1] = abi.encodeWithSelector(IPowers.assignRole.selector, 3, bob);
        calldatas[2] = abi.encodeWithSelector(IPowers.assignRole.selector, 3, DEV2_ADDRESS);
        calldatas[3] = abi.encodeWithSelector(IPowers.labelRole.selector, 1, "Users");
        calldatas[4] = abi.encodeWithSelector(IPowers.labelRole.selector, 2, "Holders");
        calldatas[5] = abi.encodeWithSelector(IPowers.labelRole.selector, 3, "Developers");
        // possibly add: subscribers. 
        // revoke law after use
        if (lawId != 0) {
            calldatas[6] = abi.encodeWithSelector(IPowers.revokeLaw.selector, lawId);
        }
