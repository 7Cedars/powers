...
contract AssignRoleWithGitSig is Law {
    ...
    function handleRequest(
        ...
        (mem.addressClaimRole, mem.lawHashClaimRole, mem.active) = Powers(payable(powers)).getAdoptedLaw(conditions.needFulfilled); 
        if (!mem.active) {
            revert ("Claim role law not active"); 
        }

        // step 3: retrieve data from chainlink reply - and reset data in the process. 
        (mem.errorMessage, mem.roleId) = ClaimRoleWithGitSig(mem.addressClaimRole).getLatestReply(mem.lawHashClaimRole, caller); 
        ...
        calldatas[0] = abi.encodeWithSelector(
            ClaimRoleWithGitSig.resetReply.selector,
            powers,
            lawId, 
            caller
        );
