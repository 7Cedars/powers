...
On-chain organizations are rapidly evolving, but their governance models are struggling to keep pace. The ecosystem's foundational vision of full autonomy, reliant on simple "one-token-one-vote" systems, is colliding with a practical reality: effective management requires stable institutional structures. As organizations mature, they are rebuilding the very specialized bodies they once sought to escape. This re-institutionalization is proving essential for complex operations, yet it introduces a profound new challenge to the core ideals of decentralization.

The challenge is that this new wave of institutionalization is occurring almost entirely off-chain. As on-chain organizations mature, they are creating a familiar array of specialized governance bodies—foundations, working groups, and councils—to manage their complex operations. Yet, these entities often have a limited on-chain footprint, and more critically, the power relations between them are seldom encoded on the ledger. This dynamic creates a shadow governance structure that operates outside the transparent and verifiable environment of the blockchain, progressively shrinking the space for genuinely autonomous on-chain decision-making and posing an existential threat to the ecosystem's core ideals.

The Powers protocol offers a unique solution that enables communities to institutionalize these power relations directly on-chain.
...
* The **Solidity Protocol**: The smart contract framework as described in this paper. The protocol has completed one audit by the Invcbull Audit Group and is deployed on multiple EVM testnets, including Mainnet Sepolia, Optimism Sepolia, and Arbitrum Sepolia. Further audits and bug bounty programs are planned, conditional on funding.  
* **dApp Interface**: A decentralized application is available for users to visualize and interact with organizations built on the Powers protocol.  
* **Example Organizations**: A set of example laws and organizational structures are available to demonstrate the protocol's capabilities. Deployed examples, which are listed below, include "Powers 101," "Power Base," and "Powers to Nouns".

## Organisations

The following are deployed example organisations that demonstrate institutional governance using the Powers protocol. 

### *Powers 101*

“Powers 101” is a simple educational DAO designed to demonstrate the protocol's core concepts. It provides a basic governance structure with a clear separation of powers. Anyone can become a Member and propose actions. An Admin role holds veto power, and an elected body of Delegates is responsible for executing approved proposals. This minimalist implementation serves as an ideal starting point for users to understand how on-chain checks and balances function within the Powers protocol.

### *Power Base*

"Power Base" is the on-chain organization that shepherds the development of the Powers protocol itself. Its primary task is distributing funds for protocol, frontend, and documentation work. It utilizes three separate Allo v2 grant pools, each managed by its respective contributor group (e.g., Protocol Contributors). Members (contributors and funders) vote on funding allocations, Funders can veto, and the Admin executes. This creates a decentralized, role-based system for managing the project's ongoing development.
