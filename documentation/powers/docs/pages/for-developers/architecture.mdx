...
<Callout type="info">
Please see the litepaper for a detailed description of how cross-law conditions work.
</Callout>
...
## 🌊 Technical Flow: `Powers.sol` and `Law.sol`
...
##### Call ExecuteLaw 
If all checks pass, it calls `Law::executeLaw` to execute an action. 

##### Translate Input to Output Function Calls 
The `Law::executeLaw` decodes input calldata, runs any optional additional checks and computes return function calls to `Powers::Fulfill`. 
