...
### Proposing an Action & Running Checks (`checksAtPropose`)

Before an action is formally proposed, the `checksAtPropose` function runs to ensure validity.
- **Base Checks**: Verifies parent law completion or block conditions.
- **Custom Checks**: Laws can implement custom logic to check role permissions, validate input parameters, and check state-dependent or time-based conditions.
...
2.  **Proposal & Execution Checks**: Calls `checksAtPropose` and `checksAtExecute` to validate all conditions.
3.  **Execution**: Calls `handleRequest` to get the action details.
4.  **State Change**: If `handleRequest` returns state change data, it calls `_changeState`.
5.  **Reply to Powers**: If `handleRequest` returns execution targets, it calls `_replyPowers`.
6.  **Tracking**: Records the execution details.

### Replying to Powers (`_replyPowers`)
