...
PresetSingleAction is an executive mandate that enables the execution of a predefined set of function calls. It provides a mechanism to execute multiple actions in a specific order with fixed parameters.
...
When adopting a PresetSingleAction instance, three parameters must be provided:
...
When calling the mandate, no parameters are required. The mandate executes the predefined set of actions in the order they were configured.
