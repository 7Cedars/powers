...
flowchart TB  
    RClass((Any Class)):::user_class  
    RFactions((All Factions)):::user_faction  
    RAdmin((Admin)):::user_admin  
    Treasury:::contract
    ...
    RClass -- Vote 66% success 50% quorum --> A  
    RFactions -- Vote 66% success 50% quorum + 2/3 Factions pass --> B  
    RAdmin -- Execute --> C

    A -- enables --> B  
    B -- enables --> C

    C -- setBudgetAllocation --> Treasury

    classDef user_class fill:#d1c4e9  
    classDef user_faction fill:#c8e6c9  
    classDef user_admin fill:#ffcdd2  
    classDef contract fill:#ffd700,stroke:#333,stroke-width:3px
```

### **Standard Grant Approval Process**

```mermaid

flowchart TD  
    RPublic((Public)):::user_public  
    RClass((Sponsoring Class)):::user_class  
    RFactions((All Factions)):::user_faction  
    Treasury:::contract
    ...
    C{Ratification Vote}  
    D[Proposal Approved]  
    E{Release Milestone Vote}  
    F

    RPublic -- Submit --> A  
    A -- triggers --> B  
    RClass -- Vote 51% success / 25% quorum --> B  
    B -- On Success --> C  
    RFactions -- Vote 51% success / 33% quorum --> C  
    C -- On Success --> D  
    D -- enables --> E  
    RClass -- Vote 51% success / 25% quorum --> E  
    E -- On Success --> F  
    F -- releaseMilestone() --> Treasury

    classDef user_public fill:#e0e0e0  
    classDef user_class fill:#d1c4e9  
    classDef user_faction fill:#c8e6c9  
    classDef contract fill:#ffd700,stroke:#333,stroke-width:3px
```
...
flowchart TB  
    RClass((Any Class)):::user_class  
    RFactions((All Factions)):::user_faction  
    RNouncil((The Nouncil)):::user_nouncil  
    RAdmin((Admin)):::user_admin
    ...
    RClass -- Vote 66% success 50% quorum --> A  
    RFactions -- Vote 66% success 50% quorum (all 3 Factions must pass) --> B  
    RNouncil -- 33% Vote to Veto (72h window) --> C  
    RAdmin -- Execute --> D

    A -- enables --> B  
    B -- enables --> C  
    C -- No Veto --> D  
    C -- Vetoed --> X[Proposal Failed]

    classDef user_class fill:#d1c4e9  
    classDef user_faction fill:#c8e6c9  
    classDef user_nouncil fill:#f8d568  
    classDef user_admin fill:#ffcdd2
```
...
flowchart TB  
    ...
    RPublic -- registerHolder() --> A  
    A -- triggers --> B  
    B -- calls --> C  
    C -- reads seeds() --> NounsToken  
    C -- returns traits --> B  
    B -- updates state --> RoleManager
