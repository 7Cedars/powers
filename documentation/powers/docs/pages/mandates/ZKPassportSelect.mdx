...
ZKPassportSelect is an electoral mandate that enables role assignment based on attestations. It allows accounts to be assigned roles only after receiving a required number of attestations from other accounts with specific attestation roles.
...
When adopting a ZKPassportSelect instance, four parameters must be provided:
