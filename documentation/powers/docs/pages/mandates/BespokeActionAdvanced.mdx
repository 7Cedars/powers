# BespokeActionAdvanced

BespokeActionAdvanced is an executive mandate that enables the execution of custom function calls with mixed static and dynamic parameters. It provides a mechanism to execute bespoke actions with pre-configured static parameters and user-provided dynamic parameters.
...
When calling the mandate, an array of dynamic parameter values must be provided. The parameters are inserted into the function call at the positions specified by `indexDynamicParams`.
