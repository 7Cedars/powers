...
OpenAction is an executive mandate that enables the execution of any function call to any contract. It provides a mechanism for unrestricted execution of arbitrary actions.
...
When adopting an OpenAction instance, no parameters are required. The mandate is completely open-ended and allows any function call to be executed.

## Usage

### Proposing an Action

When calling the mandate, three parameters must be provided:
...
```solidity
mapping(bytes32 mandateHash => bool) public initialized;
```
