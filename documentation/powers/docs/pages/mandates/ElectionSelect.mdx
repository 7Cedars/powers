...
OpenElectionEnd is an electoral mandate that enables delegation-based role assignment. It allows accounts to delegate their role assignment rights to other accounts, who can then assign or revoke roles on their behalf.
...
When adopting an OpenElectionEnd instance, three parameters must be provided:
...
```
- Returns the current state of the mandate
